function clock(){
        ...
        function mark(){
                //圆的半径
                var r=parseFloat(window.getComputedStyle?window.getComputedStyle($("clockm"),null).width:$("clockm").currentStyle["width"])/2;
                //插入DOM
                for(var i=1;i<61;i++){
                        $("clockm-mark").innerHTML+="<b class='c"+i+"'><i></i></b>";
        ...
        function clockDate(){
                ...
                $("clockm-date").innerHTML=d.getFullYear()+"年"+(d.getMonth()+1)+"月"+d.getDate()+"日"+" 星期"+week[d.getDay()];
                }
        //调用函数
        mark();
        turnR();
